...
func TestReaderLPCMMisalignedPayload(t *testing.T) {
	messages := []message.Message{
		&message.DataAMF0{
			ChunkStreamID:   4,
			MessageStreamID: 1,
	...
	tracks := r.Tracks()
	require.Equal(t, []*gortmplib.Track{{Codec: &codecs.LPCM{
		BitDepth:     16,
		SampleRate:   44100,
		ChannelCount: 2,
	}}}, tracks)
	...
	called := 0
	r.OnDataLPCM(tracks[0], func(_ time.Duration, _ []byte) {
		called++
	})
	...
	err = r.Read()
	require.EqualError(t, err, "invalid payload length: 2")
	require.Equal(t, 1, called)
}
